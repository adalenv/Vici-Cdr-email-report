...
#
# CHANGES
# ADALEN VLADI Email calls report
...
#	echo "\n";
	...
	while ( ($m < $uc) and ($m < 50000) )
		...
		if (isset($_GET['mail_report'])) {
			array_push($arr1,array('user'=>$RAWuser,'calls'=>$RAWcalls));
		}
...
if (isset($_GET['mail_report'])) {
	...
	$con=mysqli_connect('192.168.1.231','cdr',$_GET['cdr_pass'],'cdrdb');
	$astDB=mysqli_connect('192.168.1.231','cron',$_GET['asterisk_pass'],'asterisk');
	$astResult=mysqli_query($astDB,"SELECT user,full_name from vicidial_users where active ='Y'");
	$users='';
	$userArr=array();
	while ($userR=$astResult->fetch_assoc()) {
		$user=str_replace('cc','', $userR['user']) ;
		$users.="'".$user."',";
		//array_push($userArr,array($userR['user']=>$userR['full_name']));
		$userArr[$userR['user']]=$userR['full_name'];
	};
	$users=rtrim($users,',');

	$arr2=array();
	$query="SELECT  CONCAT_WS('','cc',src) AS user,count(src) AS calls FROM cdr WHERE src IN ($users) and DATE(`calldate`)=DATE('".$_GET['query_date']."') group by src  limit 500";
	//echo $query;
	//echo '----------------</br>';
	$result=mysqli_query($con,$query) or die('error');
	while ($row=$result->fetch_assoc()) {
		array_push($arr2,array('user'=>$row['user'],'calls'=>$row['calls']));
	}
	...
	$values = array();
	foreach($arr as $val) {
	    if(isset($values[$val['user']])) {
	        $values[$val['user']] += $val['calls'];
	    } else {
	        $values[$val['user']] = $val['calls'];
	    }
	}
	...
	foreach ($values as $key => $value) {
		if ($value<50) {
			$output[$userArr[$key]]=$value;
		}
	}
	//print_r($userArr);
	//print_r($output);
	$ostr='Date: '.date("Y-m-d").'</br><table><tr><td>User</td><td>Calls</td></tr>';
	foreach ($output as $k => $o) {
		$ostr.='<tr><td>'.$k.'</td><td>'.$o.'</td></tr>';
		//print_r($o);
	}
	$ostr.='</table>';
	//echo $ostr;
	...
	$headers = 'Content-Type: text/html; charset=ISO-8859-1'."\r\n".
		'From: webmaster@callsreport.com' . "\r\n" .
	    'Reply-To: webmaster@callsreport.com' . "\r\n" .
